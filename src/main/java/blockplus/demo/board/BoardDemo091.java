...
import blockplus.model.board.Remaining;
 ...
 * White color
 */
public final class BoardDemo091 {
    ...
    public static void main(final String[] args) throws InterruptedException {
        ...
        newBoard = newBoard.apply(Blue, Pieces.get(1).getInstances().getDistinctInstance(0).translateTo(Position(3, 3)));
        newBoard = newBoard.apply(Yellow, Pieces.get(1).getInstances().getDistinctInstance(0).translateTo(Position(3, 5)));
        newBoard = newBoard.apply(Red, Pieces.get(1).getInstances().getDistinctInstance(0).translateTo(Position(5, 5)));
        newBoard = newBoard.apply(Green, Pieces.get(1).getInstances().getDistinctInstance(0).translateTo(Position(5, 3)));
        ...
        // TODO ! extract RemainingSpaceView
        final Map<Remaining, Color> colorByRemainingSpace = new ImmutableMap.Builder<Remaining, Color>()
                .put(Remaining.NONE, new Color(255, 255, 255, 0))
                .put(Remaining.ONE, new Color(255, 255, 255, 60))
                .put(Remaining.TWO, new Color(255, 255, 255, 120))
                .put(Remaining.THREE, new Color(255, 255, 255, 180))
                .put(Remaining.ALL, new Color(255, 255, 255, 255))
                .build();

        final SwingRendering remainingSpaceView = new SwingRendering(colorByRemainingSpace);

        remainingSpaceView.apply(newBoard.getRemainingSpace(Blue));
