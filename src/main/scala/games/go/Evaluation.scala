...
object Evaluation {
  ...
  private def computeGlobalFreedom(board: Board, character: Char): Double = {
  ...
  def evaluateBoard(character: Char, board: Board, nextBoard: Board): Double = {
  ...
  private def evaluateOption(character: Char, board: Board, p: Position, level: Int): Double = {
  ...
  def evaluateOptions(options: Set[Position], character: Char, board: Board, level: Int) = {
